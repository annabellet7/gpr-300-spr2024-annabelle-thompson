...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glBindTextureUnit(1, stoneNormals);
		{
			...
			monkeyTransform.position = animation.anim->getPosition(deltaTime);
		...
		monkeyTransform.scale = animation.anim->getScale(deltaTime);
		monkeyTransform.rotation = animation.anim->getRotation(deltaTime);
		monkeyTransform.position = animation.anim->getPosition(deltaTime);
...
void drawUI(int tex) {
	...
	ImGui::Checkbox("Playing", &animation.anim->mIsPlaying);
	ImGui::Checkbox("Looping", &animation.anim->mIsLooping);
	ImGui::SliderFloat("Duration", &animation.anim->getClip()->mDuration, animation.anim->getClip()->mLastFrame, 100.0f);
	ImGui::SliderFloat("Playback Time", &animation.anim->mPlaybackTime, 0.0f, animation.anim->getClip()->mDuration);

	if (ImGui::CollapsingHeader("Position Key Frames"))
	{
		for (int i = 0; i < animation.anim->getClip()->mPositionKeys.size(); i++)
		{
			...
			{
				float x = 0.0f;
				ImGui::SliderFloat("Time", &x,
					0.0f, 0.0f);
				ImGui::SliderFloat3("Position", &animation.anim->getClip()->mPositionKeys[i].mValue.x, -10.0f, 10.0f);
			}
			else
			{
				ImGui::SliderFloat("Time", &animation.anim->getClip()->mPositionKeys[i].mTime,
					0.0f, animation.anim->getClip()->mDuration);
				ImGui::SliderFloat3("Position", &animation.anim->getClip()->mPositionKeys[i].mValue.x, -10.0f, 10.0f);
			}
			ImGui::PopID();
	...
	if (ImGui::Button("Add Position Key Frame"))
	{
		animation.anim->getClip()->addPositionFrame();
	}

	if (ImGui::Button("Remove Position Key Frame"))
	{
		animation.anim->getClip()->removePositionFrame();
	}

	if (ImGui::CollapsingHeader("Rotation Key Frames"))
	{
		for (int i = 0; i < animation.anim->getClip()->mRotationKeys.size(); i++)
		{
			...
			if (i == 0)
			{
				...
				ImGui::SliderFloat("Time", &x,
					0.0f, 0.0f);
				ImGui::SliderFloat3("Rotation", &animation.anim->getClip()->mRotationKeys[i].mValue.x, 0.0f, 180.0f);
			}
			else
			{
				ImGui::SliderFloat("Time", &animation.anim->getClip()->mRotationKeys[i].mTime,
					0.0f, animation.anim->getClip()->mDuration);
				ImGui::SliderFloat3("Rotation", &animation.anim->getClip()->mRotationKeys[i].mValue.x, 0.0f, 359.0f);
			}
			ImGui::PopID();
		}
	}

	if (ImGui::Button("Add Rotation Key Frame"))
	{
		animation.anim->getClip()->addRotationFrame();
	}

	if (ImGui::Button("Remove Rotation Key Frame"))
	{
		animation.anim->getClip()->removeRotationFrame();
	}
		
	if (ImGui::CollapsingHeader("Scale Key Frames"))
	{
		for (int i = 0; i < animation.anim->getClip()->mScaleKeys.size(); i++)
		{
			...
			if (i == 0)
			{
				...
				ImGui::SliderFloat("Time", &x,
					0.0f, 0.0f);
				ImGui::SliderFloat3("Scale", &animation.anim->getClip()->mScaleKeys[i].mValue.x, 0.0f, 10.0f);
			}
			else
			{
				ImGui::SliderFloat("Time", &animation.anim->getClip()->mScaleKeys[i].mTime,
					0.0f, animation.anim->getClip()->mDuration);
				ImGui::SliderFloat3("Scale", &animation.anim->getClip()->mScaleKeys[i].mValue.x, 0.0f, 10.0f);
			}
			ImGui::PopID();
	...
	if (ImGui::Button("Add Scale Key Frame"))
	{
		animation.anim->getClip()->addScaleFrame();
	}

	if (ImGui::Button("Remove Scale Key Frame"))
	{
		animation.anim->getClip()->removeScaleFrame();
	}
